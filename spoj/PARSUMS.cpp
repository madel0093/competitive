...
int main(){
	...
	while(cin>>n && n)
	{
		...
		r(n){ //// extend the partail sum to 2*n to get the cyclic partial sum data[i](Sum(data[0]:data[i%n]))
			int in=data[i];
			if(i) in-=data[i-1];
			if(data.size()) in+=data.back();
			data.push_back(in);
		...
		r(n){
			...
			if(data[val]-last<0) ans--;  //// the actual value we colud reach (by removing partial sum from 0 to i-1 )
